...
export default async function PostComponent({ post, postType }: Props) {
    ...
    return (
        <Link
            href={"/blogs/" + encodeURIComponent(story_id)}
            data-type="post"
            data-genre={genres}
            data-catagory={postType}
        >
            <div className="border overflow-hidden flex flex-col  hover:bg-[#ebc99a10] text-black  transition-colors justify-between border-black border-opacity-20 rounded-sm">
                <div className="flex justify-between items-center px-4 py-2 border-b border-black border-opacity-20 py-">
                ...
                <Image
                    ...
                    placeholder="blur"
                    width={200}
                    className="w-full h-auto aspect-[16/9] bg-theme-bg object-cover border-y border-black border-opacity-20 "
                />
                ...
                <span className="flex flex-wrap gap-1 text-sm border-t px-4 py-2 border-black border-opacity-20">
